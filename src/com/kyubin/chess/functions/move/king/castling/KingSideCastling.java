...
public class KingSideCastling {
    public static boolean isPossibleKingSideCastling(boolean is_white){
        if(is_white&&!ChessGame.white_is_possible_kingSideCastling) return false;
        if(!is_white&&!ChessGame.black_is_possible_kingSideCastling) return false;
        ...
        if(is_white){
            return !PieceFunctions.isPieceInSquare(new XY(ChessGame.white_king.xy.x + 2,
                    ChessGame.white_king.xy.y));
        } else {
            return !PieceFunctions.isPieceInSquare(new XY(ChessGame.black_king.xy.x + 2,
